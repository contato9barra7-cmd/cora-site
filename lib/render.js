...
export const CREDITOS = {
  ...
  // A análise custa o mesmo que "Ler materiais" — é o mesmo trabalho
  // (1-2 chamadas GPT), só que por cena.
  analiseBatch: 15
};
...
export async function analisarBatch({ cenas, refs }) {
  const r = await fetch(`${RENDER_URL}/analisar-batch`, {
    ...
    body: JSON.stringify({
      cenas: cenas.map((c) => ({ nome: c.nome, base64: c.base64 })),
      refs:  refs,
      tipo:  'interno',
      origem: 'web'
    })
...
export async function gerarBatch({ cenas, refs }, { onProgresso } = {}) {
  ...
  for (const cena of cenas) {
    ...
    for (let i = 0; i < cena.qtd; i++) {
      ...
      feito += 1;
      if (onProgresso) onProgresso(feito, total);
    }
