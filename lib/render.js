...
async function comToken() {
  ...
  return {
    ...
    // Em que lingua o servidor deve escrever os avisos de erro. Ver o
    // mesmo cabecalho no authFetch (lib/auth.js) e o recados.js dos dois
    // servidores.
    'X-Cora-Idioma': idiomaAtual()
  };
}
...
export const CREDITOS = {
  ...
  materiais: 16,                                  // botão "Ler materiais" (2 GPT: promptador + tradutor)
  // 1K e 2K custam o MESMO desde 03/09/2026: o Nano Banana cobra igual nas
  // duas resolucoes (so o 4K sai mais caro no provedor).
  imagem:    { '1k': 134, '2k': 134, '4k': 231 }, // imagem adicional; 1a = prompt+imagem = 142/142/239

  // (O preço fechado de batch acabou em 10/08/2026: a leitura pode vir da
  // aba Análises já paga, então o batch gera com prompt + imagem, igual ao
  // render — ver custoBatchCena.)

  // Análise do batch: 16 POR CENA (2 promptadores: série + tradutor). O
  // servidor é quem cobra (creditos.js);
  // isto aqui só MOSTRA, para a pessoa não ser surpreendida.
  analiseBatch: 16,

  // Editar (os 6 modos de texto): prompt + imagem, num pedido só.
  editar:     { '1k': 127, '2k': 127, '4k': 223 },
  ...
  // Generativa (FLUX Fill): o site NAO envia resolucao na chamada, entao o
  // servidor cai no padrao '2k' = 60. Numero unico de proposito — se um dia o
  // site passar a mandar resolucao, isto vira { '1k':52, '2k':60, '4k':72 }.
  generativa: 60,
...
// Preenchimento e expansão — preço único (veja o comentário na tabela).
export function custoGenerativa() {
  return CREDITOS.generativa;
}

// Upscale: o custo depende do fator de escala (2x→2k, 4x→4k, etc.).
export function custoUpscale(scale) {
  // Normalizacao IDENTICA a do servidor (creditos.js, custoUpscale): aceita
  // 2 / '2' / '2x' / '2k' e cai em '2k' quando nao existe. Antes o fallback
  // daqui era '4k' e o do servidor '2k' — a tela mostrava 72 e cobrava 45.
...
export function custoAnimacao(modelo, resolucao, audio, duracao) {
  ...
  // ── 720p tem preço PRÓPRIO, e mais barato ──
  // Isto aqui mapeava 720p → 1080p, "porque o fal não diferencia". O fal não,
  // mas a tabela sim: o 720p é mantido mais barato de propósito, como opção de
  // entrada (decisão comercial, ver creditos.js no servidor). Resultado: a tela
  // mostrava o preço do 1080p, o servidor cobrava o do 720p — e como a tela de
  // animação nasce em 720p, era o caso PADRÃO que estava errado, em até 13%.
  // Pagava-se para oferecer um desconto que ninguém via.
  const r = String(resolucao || '').toLowerCase();
  const rk = r.includes('4k') ? '4k' : (r.includes('720') ? '720p' : '1080p');
  const cfg = m[rk] || m['1080p'];
  if (!cfg) return 0;
  let node = cfg;
  if (cfg.sem || cfg.com) node = audio ? cfg.com : cfg.sem;
  if (!node) return 0;
  const dur = parseInt(duracao, 10) || 5;
  return creditosAnim(node.s, dur, node.p);
}

// A MESMA aritmética inteira do servidor (cora-render-server/creditos.js).
// Em ponto flutuante, `(0.05 + s*dur) * 1.5 / 0.01 * p` produz 586.4999999999999
// onde a conta dá 586.5 exato, e o arredondamento cai para o lado errado — a
// tela mostrava 1 crédito a menos que a cobrança em algumas combinações.
function creditosAnim(s, dur, p) {
  const centavos = Math.round(s * 100) * dur + 5;   // 5 = os R$0,05 do promptador
  const premio = Math.round((p || 1) * 100);        // 100 ou 115
  return Math.round(centavos * 15 * premio / 1000); // × 1,5 × premium
}
...
// O batch cobra por cena IGUAL ao render: prompt (1x) + imagem por variação.
// O preço fechado antigo embutia o prompt supondo análise sempre paga junto —
// mas a leitura pode vir da aba Análises, então o prompt é cobrado à parte.
...
// ═══════════════════════════════════════════════════════════
//  Encolher antes de enviar
//
//  O servidor aceita no máximo 100 MB por requisição, e refs + cena vão como
//  base64 dentro de UM corpo. O site mandava cada arquivo do jeito que veio:
//  5 renders em 4K + a planta passaram disso, e o Railway respondeu 413 antes
//  de a rota sequer rodar (Planta 3D, 17/08/2026). O plugin nunca viu isso
//  porque captura o print a 2048 px.
//
//  Os limites são os MESMOS que o servidor já aplica por conta própria antes
//  do Gemini (`reduzirImagemBase64`: refs 2048, base 3840) — encolher aqui só
//  antecipa o corte para o lado de cá do fio, sem mudar o resultado. Só reduz:
//  imagem que já cabe passa intacta, sem re-encodar (mime: null = "a mesma").
//  JPEG continua JPEG (não inflar em PNG); o resto sai PNG. Falhou a decodifi-
//  cação? Devolve a original — antes um 413 do que uma imagem perdida.
// ═══════════════════════════════════════════════════════════
export const LADO_MAX_REF  = 2048;   // refs, detalhes e cena da análise
export const LADO_MAX_BASE = 3840;   // imagem base da geração

export async function reduzirImagem(base64, maxLado = LADO_MAX_REF) {
  if (!base64 || typeof window === 'undefined') return { base64, mime: null };
  let url = null;
  try {
    // Blob sem tipo: o navegador descobre o formato pelos bytes (PNG, JPEG,
    // WebP...) — não dependemos do rótulo que o chamador acha que é.
    ...
    if (!w || !h || Math.max(w, h) <= maxLado) return { base64, mime: null };

    const esc = maxLado / Math.max(w, h);
    const c = document.createElement('canvas');
    c.width  = Math.round(w * esc);
    c.height = Math.round(h * esc);
    c.getContext('2d').drawImage(img, 0, 0, c.width, c.height);

    const jpeg = base64.startsWith('/9j/');           // assinatura do JPEG
    const mime = jpeg ? 'image/jpeg' : 'image/png';
    const saida = jpeg ? c.toDataURL(mime, 0.92) : c.toDataURL(mime);
    return { base64: saida.split(',')[1], mime };
  } catch {
    return { base64, mime: null };
  } finally {
    if (url) URL.revokeObjectURL(url);
  }
}

// Lista de referências { base64, mimeType, ... } → mesma lista, encolhida.
// Quando a imagem foi re-encodada, o mimeType acompanha o formato novo.
async function reduzirRefs(lista, maxLado = LADO_MAX_REF) {
  return Promise.all((lista || []).map(async (r) => {
    if (!r || !r.base64) return r;
    const red = await reduzirImagem(r.base64, maxLado);
    return { ...r, base64: red.base64, mimeType: red.mime || r.mimeType };
  }));
}
...
export function assinaturaConfig(cfg) {
  return JSON.stringify([
    ...
    // Planta baixa: a PROJEÇÃO e o tipo de leitura mudam o prompt. Sem eles na
    // assinatura, trocar de ortogonal p/ isométrica (mesma imagem) reusava o
    // lote anterior e vinha o prompt ortogonal de novo.
    cfg.plantaModo, cfg.plantaLeitura,
...
export async function gerarRender(cfg, { onProgresso, loteAnterior } = {}) {
  ...
  // Encolhe UMA vez, antes do laço: as mesmas imagens vão em todas as variações.
  // (Depois da assinatura de propósito — ela lê o começo do base64 original.)
  const imagemEnvio = (await reduzirImagem(cfg.imagem, LADO_MAX_BASE)).base64;
  const refsEnvio   = await reduzirRefs(cfg.referencias);

  for (let i = 0; i < total; i++) {
    const aviso = (estado) => onProgresso && onProgresso(i, total, estado);

    // Ordem no FEED = base0 + nº de sucessos até aqui, NÃO base0 + i. Com i, uma
    // falha no meio deixava um "buraco" e, como `quantas` conta só sucessos, a
    // rodada seguinte (base0 = quantas) reusava uma ordem já ocupada por uma
    // imagem boa — duas imagens disputando a mesma posição no mesmo loteId.
    // Contando sucessos, as ordens ficam contíguas e a continuação nunca colide.
    const ordemFeed = base0 + imagens.length;

    // A Planta baixa e uma ferramenta propria no historico: sem isto ela
    // ia como 'render' e aparecia no feed, no filtro e nos Detalhes com
    // esse nome. O plugin manda a mesma palavra.
    const ferramenta = cfg.tipo === 'planta' ? 'planta' : 'render';
    const base = {
      image:       imagemEnvio,
      proporcao:   cfg.proporcao,
      resolucao:   cfg.resolucao,
      referencias: refsEnvio,
      ferramenta,
      loteId,
      ordem: ordemFeed
    };
    ...
    try {
      const job = (promptReuso === null)
        ? await postComPolling('render', {
            ...
            // Planta baixa 2D: projeção (orto/iso) e tipo de leitura (iso/persp).
            // Para os outros tipos vão como undefined — o servidor ignora.
            plantaModo:    cfg.plantaModo,
            plantaLeitura: cfg.plantaLeitura,
            // Planta 3D (web): refs definem materiais/luz da cena enviada.
            modoSerie:     cfg.modoSerie === true,
            gerarImagem:   true
          }, { onEstado: aviso })
      ...
      // A ordem do feed é lida ANTES do push (o push é que a "consome").
      if (job.imagem) imagens.push(job.imagem);
      // Passa a imagem pronta + lote/ordem: o slot a mostra NA HORA, sem esperar
      // o lote inteiro (e sem recarregar o feed — isso duplicava: slot + feed).
      if (onProgresso) onProgresso(i + 1, total, 'pronto', { ordem: ordemFeed, url: job.imagem, loteId });
...
// Atmosfera / Clima da cena (multi-seleção) — entra no prompt junto da luz natural.
export const ATMOSFERAS = [
  'Céu azul limpo', 'Nuvens leves', 'Nublado difuso',
  'Pós-chuva com reflexos sutis', 'Leve neblina', 'Clima tropical seco'
];
...
export async function editarImagem({
  ...
  // Agrupa as N variações num lote e liga cada slot ao item real do feed
  // (o servidor lê body.loteId/ordem para salvar juntas).
  const loteId  = novoLoteId('ed');
  const imagens = [];
  const falhas  = [];
  let prompt    = '';
  ...
  for (let i = 0; i < total; i++) {
    try {
      ...
      // Imagem pronta no slot na hora (mesma lógica do Render/Batch).
      if (onProgresso) onProgresso(i + 1, total, 'ok', { ordem: i, url: job.imagem, loteId });
  ...
  return { imagens, prompt, falhas, loteId };
}
...
//
//  A IA cruza cada cena com as referências e deduz os materiais. Custa 16 por
//  cena (CREDITOS.analiseBatch, que bate com o analise_batch=16 do servidor
//  em cora-render-server/creditos.js).
//
//  Antes isto era de graça, e o custo do GPT saía do bolso da Marilia.
// ═══════════════════════════════════════════════════════════
export async function analisarBatch({ cenas, refs, origemTipo, tipo }) {
  // Tudo encolhido a 2048 px antes de sair (ver reduzirImagem): a análise é só
  // visão do GPT, que de qualquer forma não olha acima disso.
  const cenasEnvio = await Promise.all(cenas.map(async (c) => ({
    nome:   c.nome,
    base64: (await reduzirImagem(c.base64)).base64
  })));
  const refsEnvio = await Promise.all((refs || []).map(async (b) => (await reduzirImagem(b)).base64));
  ...
  const r = await fetch(`${RENDER_URL}/analisar-batch`, {
    ...
    body: JSON.stringify({
      ...
      cenas: cenasEnvio.map((c) => ({
        nome:        c.nome,
        imageBase64: c.base64
      })),
      refs:  refsEnvio,     // aqui sim são strings puras
      tipo:  tipo || 'interno',
      // origemTipo='planta' faz a leitura salvar na aba Análises como "Planta
      // baixa" (a rota lê req.body.origemTipo). Sem ele, cai como 'batch'.
      origemTipo: origemTipo || undefined,
      origem: 'web'
    })
  });
  ...
  if (!r.ok) {
    ...
    // O servidor SEMPRE responde `{ erro }`. Quando não vem, a resposta não é
    // dele: é o proxy do Railway (502 "Application failed to respond" quando o
    // processo caiu; timeout de 5 min quando a análise demorou demais). Esses
    // vêm como `{ message }` ou HTML — mostrar o status e a mensagem em vez de
    // um texto genérico é a única forma de saber o que aconteceu do lado de cá.
    if (e.erro) throw new Error(e.erro);
    const detalhe = e.message ? `, ${e.message}` : '';
    throw new Error(`Não foi possível analisar as cenas (servidor respondeu ${r.status}${detalhe})`);
  }
...
export async function gerarBatch({ cenas, refs }, { onProgresso } = {}) {
  ...
  // As refs são as mesmas para todas as cenas: encolhe uma vez, fora do laço.
  const refsEnvio = await reduzirRefs(refs.map((b) => ({ base64: b, mimeType: 'image/png' })));

  for (const cena of cenas) {
    // Reaproveita o lote da geração anterior DESTA cena (quando o texto não
    // mudou), pra as novas variações caírem no MESMO agrupamento do feed.
    // Sem lote anterior (1ª vez ou texto mudou) → lote novo.
    const loteId = cena.loteId || ('bt' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6));
    const ordemBase = cena.ordemInicial || 0;   // continua a numeração do lote

    const imagens = [];
    // Reusa o prompt da geração anterior (se veio) — assim "gerar mais uma"
    // mantém o MESMO prompt/estilo e não regenera texto à toa.
    let promptReuso = cena.promptReuso || null;

    const base = {
      image:       (await reduzirImagem(cena.base64, LADO_MAX_BASE)).base64,
      referencias: refsEnvio,
      // Referências específicas desta cena (@det) — só desta cena, não do projeto.
      detalhes:    await reduzirRefs((cena.detalhes || []).map((b) => ({ base64: b, mimeType: 'image/png' }))),
      detalheTexto: cena.detalheTexto || '',
    ...
    for (let i = 0; i < cena.qtd; i++) {
      ...
      const job = (promptReuso === null)
        ? await postComPolling('render', {
            ...
            // Batch É o modo série: o servidor escolhe o promptador
            // "Renderizar Imagens do Mesmo Projeto 2.0" por esta flag. Antes
            // funcionava por acidente (o servidor usava a identidade de série
            // para TODO render); com o roteamento consertado lá, sem a flag o
            // batch cairia no promptador de print.
            modoSerie:   true,
      ...
      // não o da primeira cena o tempo todo. E a imagem RECÉM-PRONTA (base64)
      // vai também, pro slot exibi-la na hora, sem esperar o feed recarregar.
      if (onProgresso) onProgresso(feito, total, cena, job.imagem, loteId, ordem);
    }

    // Devolve loteId + prompt + próxima ordem, pra a próxima geração DESTA
    // cena reaproveitar (mesmo agrupamento).
    lotes.push({ loteId, nome: cena.nome, imagens, promptReuso, feitas: ordemBase + cena.qtd });
  }
...
//  ordem narrativa (só os números). Custa 8 (CREDITOS.narrativa).
//  Etapa 2 (roteiro): manda as imagens JÁ na ordem, a IA devolve os takes
//  (câmera, close, transição, frames 1/2), o ritmo e a trilha. Custa outros 8.
//  Fluxo completo = 16, que é o número da planilha e da página de preços.
//  (Este comentário dizia "Grátis" nas duas etapas — nunca foi.)
//  As animações dos takes custam à parte (animarKling).
// ═══════════════════════════════════════════════════════════
...
export async function narrativaRoteiro(imagens, idioma = 'pt') {
  ...
  return { takes: d.takes, ritmo: d.ritmo || '', trilha: d.trilha || '', ideia: d.ideia || '', narracao: d.narracao || '' };
}
