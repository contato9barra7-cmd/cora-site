...
export function custoUpscale(scale) {
  const chave = ({ 2: '2k', 4: '4k', 8: '8k', 16: '16k' })[scale] || '4k';
  return CREDITOS.upscale[chave] || CREDITOS.upscale['4k'];
}
...
async function timelapseUmaImagem({ image, prompt, proporcao, resolucao }, { intervalo = 4000, maxTentativas = 200 } = {}) {
  const r = await fetch(`${RENDER_URL}/timelapse-imagem`, {
    method: 'POST',
    headers: comToken(),
    body: JSON.stringify({ image, prompt, proporcao, resolucao, origem: 'web' })
  });
  if (r.status === 402) throw new Error('Créditos insuficientes para a próxima etapa');
  if (!r.ok) {
    const e = await r.json().catch(() => ({}));
...
export async function gerarTimelapse(
  ...
  for (let i = 0; i < etapas.length; i++) {
    ...
    const prompt = etapas[i].prompt || etapas[i];
    const b64 = await timelapseUmaImagem({ image: base, prompt, proporcao, resolucao });
    saidas.push(b64);
