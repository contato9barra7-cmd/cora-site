...
export async function criarNarrativa({ imagens, ordem, takes, ritmo, trilha, ideia, finalizado, plataforma }) {
  try {
    const r = await fetch(`${RENDER_URL}/narrativas`, {
      method: 'POST',
      headers: await comToken(),
      body: JSON.stringify({ imagens, ordem, takes, ritmo, trilha, ideia, finalizado, plataforma: plataforma || 'web' })
    });
    if (!r.ok) return null;
    const d = await r.json();
    return d.id || null;
...
export async function atualizarNarrativa(id, { ordem, takes, ritmo, trilha, ideia, finalizado }) {
  if (!id) return false;
  try {
    const r = await fetch(`${RENDER_URL}/narrativas`, {
      method: 'POST',
      headers: await comToken(),
      body: JSON.stringify({ id, ordem, takes, ritmo, trilha, ideia, finalizado })
    });
    return r.ok;
