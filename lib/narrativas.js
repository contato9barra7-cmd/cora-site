...
export async function listarNarrativas(limite = 40) {
  try {
    const r = await fetch(`${RENDER_URL}/narrativas?limite=${limite}`, { headers: await comToken() });
    if (!r.ok) return [];
...
export async function criarNarrativa({ imagens, ordem, takes, ritmo, trilha, ideia, narracao, finalizado, plataforma }) {
  try {
    const r = await fetch(`${RENDER_URL}/narrativas`, {
      method: 'POST',
      headers: await comToken(),
      body: JSON.stringify({ imagens, ordem, takes, ritmo, trilha, ideia, narracao, finalizado, plataforma: plataforma || 'web' })
    });
    if (!r.ok) return null;
    const d = await r.json();
    return d.id || null;
...
export async function atualizarNarrativa(id, { ordem, takes, ritmo, trilha, ideia, narracao, finalizado }) {
  if (!id) return false;
  try {
    const r = await fetch(`${RENDER_URL}/narrativas`, {
      method: 'POST',
      headers: await comToken(),
      body: JSON.stringify({ id, ordem, takes, ritmo, trilha, ideia, narracao, finalizado })
    });
    return r.ok;
...
export async function pegarNarrativa(id) {
  try {
    const r = await fetch(`${RENDER_URL}/narrativas/${id}`, { headers: await comToken() });
    if (!r.ok) return null;
    return await r.json();
  } catch (e) { return null; }
}
...
export async function apagarNarrativa(id) {
  try {
    const r = await fetch(`${RENDER_URL}/narrativas/${id}`, { method: 'DELETE', headers: await comToken() });
    return r.ok;
  } catch (e) { return false; }
}
