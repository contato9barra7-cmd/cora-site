...
//  O token e o mesmo do login — obtido via getAccessToken() (o JWT vive num
//  cookie httpOnly, NAO no localStorage; ver lib/auth.js).
// ═══════════════════════════════════════════════════════════
import { tg, idiomaAtual, localeDeIdioma } from './i18n';
...
/* ── AS IMAGENS DE OUTRA CONTA, PARA A FICHA DO ADMIN ──
   Bate na mesma casa do feed, porque e ali que as imagens moram, e usa o
   MESMO token: o render-server confere o `is_admin` de dentro dele.

   Isto abre o arquivo privado de alguem. O servidor grava um evento com quem
   abriu antes de responder, e a gravacao mora LA e nao aqui: se ela morasse
   neste arquivo, bastaria chamar a rota direto para ver sem deixar rastro. */
export async function adminGeracoesDaConta(contaId, limite = 24) {
  const r = await fetch(
    `${RENDER_URL}/admin/geracoes/${contaId}?limite=${encodeURIComponent(limite)}`,
    { headers: await comToken() }
  );
  if (r.status === 401) throw new Error('Sessão expirada. Entre novamente.');
  if (r.status === 403) throw new Error('Esta conta não tem acesso ao admin.');
  if (!r.ok) {
    const e = await r.json().catch(() => ({}));
    throw new Error(e.erro || 'Não foi possível carregar as imagens desta conta');
  }
  const dados = await r.json();
  return dados.lotes || [];
}
...
export function tempoRelativo(iso) {
  ...
  // Monta "há 5 min" (pt/es: prefixo) ou "5 min ago" (en: sufixo).
  const rel = (n, unidade) => {
    if (idiomaAtual() === 'en') return `${n} ${unidade} ${tg('tempo_ago')}`;
    return `${tg('tempo_ha')} ${n} ${unidade}`;
  };

  if (seg < 60)    return tg('tempo_agora');
  if (seg < 3600)  return rel(Math.floor(seg / 60), tg('tempo_min'));
  if (seg < 86400) return rel(Math.floor(seg / 3600), tg('tempo_h'));
  if (seg < 172800) return tg('tempo_ontem');

  const dias = Math.floor(seg / 86400);
  if (dias < 7) return rel(dias, tg('tempo_dias'));

  return data.toLocaleDateString(localeDeIdioma(idiomaAtual()), { day: '2-digit', month: 'short' });
}

// Quantos dias faltam para a geracao ser apagada (lifecycle de 60 dias no R2).
// Devolve null se ainda falta muito (nao vale avisar).
//
// SESSENTA, e nao noventa: quem apaga e a faxina do `cora-render-server`
// (index.js), e la o prazo e 60, o mesmo que a politica de privacidade
// promete. Com 90 aqui a tela dizia "apagada em 12 dias" numa imagem que ja
// tinha sumido do R2 havia um mes.
...
// Substitui os bytes de uma geração (edição na Pós vinda do feed).
export async function substituirGeracao(id, base64) {
  const limpo = String(base64 || '').replace(/^data:[^,]+,/, '');
  const r = await fetch(`${RENDER_URL}/geracoes/${id}/substituir`, {
    method: 'POST',
    headers: { ...(await comToken()), 'Content-Type': 'application/json' },
    body: JSON.stringify({ imagemBase64: limpo })
  });
  if (!r.ok) {
    const e = await r.json().catch(() => ({}));
    throw new Error(e.erro || 'Não foi possível substituir a imagem');
  }
  return r.json();
}
