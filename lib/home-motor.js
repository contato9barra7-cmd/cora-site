...
//  MOTOR DA HOME
//
//  A partir de 13/09/2026, o motor da Home suporta internacionalização via parâmetro textos.
...
export function iniciarHome(numeros, textos) {
  ...
  try {
    ...
    (function(){
      ...
      var semMov = false;
      var ferramentas = (textos && textos.ferramentas) || [
      ...
      var rot = (textos && textos.rot) || { imagem:'Imagem', video:'Vídeo', edicao:'Edição' };
      ...
      if (palco && !palco.querySelector('.passos__toque')){
        ...
        var ariaAnt = (textos && textos.passo_ant) || 'Passo anterior';
        var ariaProx = (textos && textos.passo_prox) || 'Próximo passo';
        toquePasso.innerHTML = '<button class="passos__toque-ant" type="button" aria-label="' + ariaAnt + '"></button><button class="passos__toque-prox" type="button" aria-label="' + ariaProx + '"></button>';
        ...
        if (visor && canvas){
          ...
          if (ctx){
            ...
            visor.addEventListener('keydown', function(e){
              if (/^Arrow/.test(e.key)) visor.dataset.mexeu = 'true';
      ...
      (function(){
        ...
        function contar(){
          ...
          function passo(t){
            ...
            var loc = (typeof window !== 'undefined' && window.__cora_idioma === 'en') ? 'en-US' : (typeof window !== 'undefined' && window.__cora_idioma === 'es') ? 'es-ES' : 'pt-BR';
            alvo.textContent = Math.round(x).toLocaleString(loc);
