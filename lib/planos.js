...
export const planos = [
  {
    ...
    feats: [
      ...
      [true, 'Pós-produção (apenas no teste de 7 dias)'],
...
export const recargas = [
  { id: 'p', n: 'Recarga P', creditos: 2000, preco: 30, popular: false },
  { id: 'm', n: 'Recarga M', creditos: 5000, preco: 65, popular: false },
  { id: 'g', n: 'Recarga G', creditos: 12000, preco: 140, popular: true },
  { id: 'gg', n: 'Recarga GG', creditos: 30000, preco: 320, popular: false },
];
...
export const comparacao = [
  ...
  ['grupo', 'Pós e upscale'],
  ['Pós-produção', '7 dias', false, true, true],
...
// ----------------------------------------------------------------------------
//  CUSTO POR GERAÇÃO (em créditos)
//  Organizado em 4 abas. Cada aba tem: título, colunas (cabeçalho) e linhas.
//  ATENÇÃO: as colunas de imagens/timelapse são RESOLUÇÕES (1K/2K/4K),
//  não planos. Animação são durações (1s/5s/10s).
// ----------------------------------------------------------------------------
export const custos = {
  imagens: {
    label: 'Imagens',
    colunas: ['Geração', '1K', '2K', '4K'],
    linhas: [
      ['Render', 142, 164, 239],
  ...
  timelapse: {
    ...
    colunas: ['Geração', '1K', '2K', '4K'],
    linhas: [
      ['Timelapse externo (completo)', 968, 1113, 1736],
      ['Timelapse externo (etapa)', 120, 138, 216],
      ['Timelapse interno (completo)', 968, 1113, 1736],
      ['Timelapse interno (etapa)', 120, 138, 216],
    ],
  },
  upscale: {
    label: 'Upscale',
    colunas: ['Resolução final', 'Créditos'],
    linhas: [
      ['2K', 45],
      ['4K', 72],
      ['8K', 135],
  ...
  animacao: {
    label: 'Animação',
    colunas: ['Modelo', '1s', '5s', '10s'],
    linhas: [
      ['Kling 2.1 (720p)', 96, 448, 890],
      ['Kling 2.1 (1080p)', 110, 516, 1023],
      ['Kling 2.5 (720p)', 70, 322, 638],
      ['Kling 2.5 (1080p)', 81, 371, 733],
      ['Kling 2.6 (720p, sem áudio)', 70, 322, 638],
      ['Kling 2.6 (1080p, sem áudio)', 81, 371, 733],
      ['Kling 2.6 (720p, com áudio)', 134, 638, 1268],
      ['Kling 2.6 (1080p, com áudio)', 154, 733, 1458],
      ['Kling 3.0 (720p, sem áudio)', 108, 510, 1012],
      ['Kling 3.0 (720p, com áudio)', 159, 765, 1522],
      ['Kling 3.0 (1080p, sem áudio)', 124, 587, 1164],
      ['Kling 3.0 (1080p, com áudio)', 183, 880, 1751],
...
// ----------------------------------------------------------------------------
//  CORA TEAMS — tabelas de assentos com desconto por quantidade
//  Cada linha: [faixa de assentos, desconto (0-1), preço por assento, total no mínimo]
//  total = null quando não se aplica (só o mínimo mostra total).
// ----------------------------------------------------------------------------
export const teamsPro = [
  ['2 (mínimo)', 0.05, 282, 564],
  ['3 a 4', 0.10, 267, null],
  ['5 a 9', 0.15, 252, null],
  ['10 ou mais', 0.20, 238, null],
];

export const teamsStudio = [
  ['2 (mínimo)', 0.05, 662, 1324],
  ['3 a 4', 0.10, 627, null],
  ['5 a 9', 0.15, 592, null],
  ['10 ou mais', 0.20, 558, null],
];

// ----------------------------------------------------------------------------
//  CORA TEAMS — helper de cálculo de preço por faixa de assentos
// ----------------------------------------------------------------------------
// Preço base por assento de cada plano.
// mensal = valor por mês; anual = valor por ANO por assento (2 meses grátis ≈ 17% off).
// [AJUSTAR AQUI quando a consultoria definir os valores finais]
export const teamsBase = { pro: 297, studio: 697 };
export const teamsBaseAnual = { pro: 2970, studio: 6970 };

// Retorna o desconto (0-1) conforme a quantidade de assentos.
export function descontoAssentos(qtd) {
  if (qtd >= 10) return 0.20;
  if (qtd >= 5) return 0.15;
  if (qtd >= 3) return 0.10;
  return 0.05; // 2 (mínimo)
}

// Calcula preço da equipe: { porAssento, total, desconto, economia }
// ciclo = 'mensal' | 'anual'
