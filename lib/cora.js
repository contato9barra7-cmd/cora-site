...
// ── Onde salvar ──
//
// O navegador moderno tem um seletor de arquivo DE VERDADE: a mesma janela de
// "Salvar como" do sistema, onde a pessoa escolhe a pasta e o nome. É o que se
// espera de um programa que salva projetos.
//
// Ele só existe no Chrome e no Edge. No Firefox e no Safari a função nem está
// definida, e a saída é o download comum — que cai em Downloads com o nome que
// nós dermos. Menos bom, mas não quebra nada.
async function ondeSalvar(zip, nome) {
  const arquivo = `${nome}.crd`;
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      // A pessoa cancelou a janela: não é erro, e não há nada a fazer.
...
export async function exportarCora(camadas, med, selecao, nome = 'trabalho') {
  ...
  await ondeSalvar(zip, nome.replace(/[^\w\- ]/g, '_').trim() || 'trabalho');
}
...
export async function importarCora(file) {
  ...
  for (const c of projeto.camadas) {
    ...
    l.id = novosIds[c.id];
    l.grupo = c.grupo ? (novosIds[c.grupo] || null) : null;
