import { novoId, novoGrupo } from './pos';
// ═══════════════════════════════════════════════════════════
//  O arquivo .crd — Cora Render Document
//
...
function lerZip(buffer) {
  ...
  while (i < b.length - 4) {
    ...
    if (metodo !== 0) throw new Error('arquivo .crd não reconhecido');
...
function paraPng(canvas) {
  return new Promise((ok, erro) => {
    canvas.toBlob(async (blob) => {
      // toBlob chama o callback com null quando a codificação falha (canvas
      // gigante / sem memória). Sem este guard, blob.arrayBuffer() lançava
      // DENTRO do callback async e a Promise NUNCA resolvia — o export inteiro
      // do .crd (await por camada) travava em silêncio.
      if (!blob) { erro(new Error('Falha ao codificar PNG (canvas grande demais?)')); return; }
      try { ok(new Uint8Array(await blob.arrayBuffer())); }
      catch (e) { erro(e); }
    }, 'image/png');
  });
}
...
export async function exportarCora(camadas, med, selecao, nome = 'trabalho') {
  ...
  const arquivoNome = `${(nome || '').replace(/[^\w\- ]/g, '_').trim() || 'trabalho'}.crd`;

  // ── A janela de "Salvar como" vem ANTES da montagem ──
  //
  // O seletor de verdade (Chrome/Edge) abre já no clique, por duas razões. A
  // primeira é a espera: num projeto grande os paraPng somam segundos, e
  // montar tudo antes de qualquer janela parecia travamento — a mesma
  // inversão foi feita no plugin. A segunda é dura: o showSaveFilePicker
  // exige um gesto RECENTE do usuário, então montar primeiro podia fazer o
  // navegador recusar a janela (SecurityError) justamente nos projetos
  // grandes. Cancelou, nada é montado.
  //
  // No Firefox e no Safari a função nem existe: monta e baixa em Downloads,
  // como sempre foi.
  let alvo = null;
  if (window.showSaveFilePicker) {
    try {
      alvo = await window.showSaveFilePicker({
        suggestedName: arquivoNome,
        types: [{
          description: 'Projeto do Cora Render',
          accept: { 'application/x-cora-render-document': ['.crd'] }
        }]
      });
    } catch (e) {
      // A pessoa cancelou a janela: não é erro, e nada chega a ser montado.
      if (e.name === 'AbortError') return;
      alvo = null;   // qualquer outra falha cai no download comum, no fim
    }
  }
  ...
  for (const l of camadas) {
    const c = {
      ...
      // A RECEITA dos filtros inteligentes. Sem ela, salvar e reabrir perderia
      // a reversibilidade: o canvas viria com os efeitos carimbados, e não
      // haveria mais como voltar num desfoque para mexer no raio.
      //
      // As máscaras são canvases: cada uma vira um PNG à parte.
      filtros: (l.filtros || []).map((f) => ({
        id: f.id,
        tipo: f.tipo,
        raio: f.raio,
        angulo: f.angulo,
        params: f.params || null,
        desligado: f.desligado || false,
        temMascara: !!f.mascara
      }))
    };
    ...
    if (l.tipo !== 'grupo') {
      ...
      // A máscara de cada filtro, uma por uma.
      for (const f of (l.filtros || [])) {
        if (!f.mascara) continue;
        arquivos.push({
          nome: `camadas/${l.id}-f-${f.id}.png`,
          dados: await paraPng(f.mascara)
        });
      }
    }
  ...
  if (alvo) {
    ...
    await fluxo.write(zip);
    await fluxo.close();
    return;
  }

  // Download comum (Firefox/Safari, ou um picker que falhou sem ser cancelado).
...
export async function importarCora(file) {
  ...
  if (!mapa) throw new Error('arquivo .crd inválido');

  const projeto = JSON.parse(new TextDecoder().decode(mapa));

  if (projeto.formato !== ASSINATURA) throw new Error('arquivo .crd inválido');
  ...
  //
  // O de-para é guardado porque os grupos referenciam os filhos por id: sem ele,
  // os grupos apontariam para camadas que já não existem.
  const novosIds = {};
  for (const c of projeto.camadas) novosIds[c.id] = novoId();

  const camadas = [];

  // ── Gid órfão: o grupo do PLUGIN ──
  //
  // Lá, grupo não é camada — é só marcação (gid 'g5' nos membros, nome num
  // mapa). O .crd salvo no plugin traz os membros apontando para um gid que
  // não existe em `camadas`, e o `novosIds[c.grupo] || null` derrubava o
  // vínculo: o projeto abria com tudo solto. Aqui o grupo vira entrada de
  // verdade (o modelo daqui), sintetizada antes do primeiro membro — a mesma
  // posição que o agrupar() usa. O nome se perde no formato do plugin (ele
  // não grava nomes de grupo no arquivo); fica o padrão.
  const gruposOrfaos = {};
  let nOrfaos = 0;
  const grupoDoMembro = (gidArq) => {
    if (novosIds[gidArq]) return novosIds[gidArq];
    if (!gruposOrfaos[gidArq]) {
      const g = novoGrupo('Grupo' + (++nOrfaos > 1 ? ' ' + nOrfaos : ''));
      gruposOrfaos[gidArq] = g;
      camadas.push(g);
    }
    return gruposOrfaos[gidArq].id;
  };
  ...
  for (const c of projeto.camadas) {
    ...
    if (c.tipo !== 'grupo') {
      ...
      if (!px) throw new Error('arquivo .crd incompleto');
      ...
      // Os filtros, com as máscaras de volta ao formato de canvas.
      l.filtros = [];
      for (const f of (c.filtros || [])) {
        const g = { ...f };
        delete g.temMascara;

        g.mascara = f.temMascara
          ? await dePng(arquivos[`camadas/${c.id}-f-${f.id}.png`])
          : null;
    ...
    l.id = novosIds[c.id];
    l.grupo = c.grupo ? grupoDoMembro(c.grupo) : null;
