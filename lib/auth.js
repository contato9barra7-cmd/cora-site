...
export async function renomearEquipe(nome) {
  const token = lerToken();
  ...
  const dados = await r.json();
  if (!r.ok) throw new Error(dados.erro || 'Erro ao renomear');
  return dados.nome;
}
...
export async function minhaEquipe() {
  const token = lerToken();
  if (!token) return null;
  const r = await fetch(`${AUTH_URL}/minha-equipe`, { headers: { 'Authorization': 'Bearer ' + token } });
  const dados = await r.json();
  if (!r.ok) return null;
  return dados.membro;
}
