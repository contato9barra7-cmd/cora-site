...
// ── A curva ──
// Interpolação linear entre os pontos → uma LUT de 256 entradas. Consultar uma
// tabela é o que torna a curva barata: a conta é feita 256 vezes, não uma vez
// por pixel.
function curvaLUT(pts) {
  ...
  for (let x = 0; x < 256; x++) {
    ...
    for (let i = 0; i < s.length - 1; i++) {
      if (x >= s[i].x && x <= s[i + 1].x) { p0 = s[i]; p1 = s[i + 1]; break; }
    }

    const t = (p1.x === p0.x) ? 0 : (x - p0.x) / (p1.x - p0.x);
    lut[x] = Math.max(0, Math.min(255, Math.round(p0.y + (p1.y - p0.y) * t)));
  }
  return lut;
}
