...
export function temAjuste(p) {
  if (!p) return false;
  const q = (o) => o ? Object.values(o).some((v) => v !== 0) : false;
  ...
  const e = p.efeitos;
  if (e && (e.textura || e.clareza || e.dehaze || e.vinheta || e.grao)) return true;

  if (p.mixer && mixerTemAjuste(p.mixer)) return true;
  if (p.curva && curvaMexida(p.curva)) return true;
...
export function curvaLUT(pts) {
  ...
  // Spline cúbica NATURAL pelos pontos, que é a curva do Photoshop: passa
  // lisa por todos, sem cotovelo. As segundas derivadas vêm do sistema
  // tridiagonal com as pontas livres (M[0] = M[n-1] = 0). Antes era a
  // monótona de Fritsch–Carlson, que achata a tangente onde a curva vira e
  // deixava o S duro. O que a natural passa de 0 ou 255 é cortado.
  const xs = s.map((p) => p.x), ys = s.map((p) => p.y);
  const h = [];
  for (let i = 0; i < n - 1; i++) h[i] = Math.max(1e-6, xs[i + 1] - xs[i]);

  const M = new Array(n).fill(0);
  if (n > 2) {
    const u = new Array(n).fill(0), z = new Array(n).fill(0);
    let l = 1, mu = 0;
    for (let i = 1; i < n - 1; i++) {
      const alpha = (3 / h[i]) * (ys[i + 1] - ys[i]) - (3 / h[i - 1]) * (ys[i] - ys[i - 1]);
      l = 2 * (xs[i + 1] - xs[i - 1]) - h[i - 1] * mu;
      mu = h[i] / l;
  ...
  for (let x = 0; x < 256; x++) {
    ...
    if (x <= xs[0]) y = ys[0];
      ...
      const t = x - xs[k], hk = h[k];
      const a3 = (M[k + 1] - M[k]) / (3 * hk);
      const b2 = M[k];
      const c1 = (ys[k + 1] - ys[k]) / hk - hk * (2 * M[k] + M[k + 1]) / 3;
      y = ys[k] + c1 * t + b2 * t * t + a3 * t * t * t;
    }
...
// Devolve o ajuste EFETIVO {h,s,l} deste pixel — interpolação linear entre as
// DUAS faixas de cor adjacentes (partição da unidade, estilo Lightroom) e
// atenuação por saturação (satW). NÃO aplica: só devolve a força. Esses campos
// são suavizados depois, o que mata o granulado.
function mixerPesos(r, g, b, mix) {
  const hsl = rgb2hsl(r, g, b);
  const hh = hsl[0] * 360;
  const satW = smooth01((hsl[1] - 0.02) / 0.06);   // tons quase neutros recebem menos
  if (satW <= 0) return null;

  const A = FAIXAS;                 // [nome, ângulo], já em ordem crescente
  let lo = A[A.length - 1], hi = A[0], loH = A[A.length - 1][1], hiH = A[0][1] + 360;
  for (let k = 0; k < A.length; k++) {
    const cur = A[k][1], nxt = (k + 1 < A.length) ? A[k + 1][1] : A[0][1] + 360;
    if (hh >= cur && hh < nxt) { lo = A[k]; hi = (k + 1 < A.length) ? A[k + 1] : A[0]; loH = cur; hiH = nxt; break; }
  }
  const span = hiH - loH, wHi = span > 0 ? (hh - loH) / span : 0, wLo = 1 - wHi;
  const ajLo = mix[lo[0]] || {}, ajHi = mix[hi[0]] || {};
  return {
    h: ((ajLo.h || 0) * wLo + (ajHi.h || 0) * wHi) * satW,
    s: ((ajLo.s || 0) * wLo + (ajHi.s || 0) * wHi) * satW,
    l: ((ajLo.l || 0) * wLo + (ajHi.l || 0) * wHi) * satW
  };
}

// Box blur separável de UM canal Float32 (bordas grampeadas). Usado só nos
// campos de força do misturador — não na imagem.
function blurCanal(a, w, h, rad) {
  const tmp = new Float32Array(a.length), div = rad * 2 + 1;
  for (let y = 0; y < h; y++) {
    const base = y * w;
    for (let x = 0; x < w; x++) {
      let s = 0;
      for (let k = -rad; k <= rad; k++) { let xx = x + k; if (xx < 0) xx = 0; else if (xx >= w) xx = w - 1; s += a[base + xx]; }
      tmp[base + x] = s / div;
    }
  }
  for (let x = 0; x < w; x++) {
    for (let y = 0; y < h; y++) {
      let s = 0;
      for (let k = -rad; k <= rad; k++) { let yy = y + k; if (yy < 0) yy = 0; else if (yy >= h) yy = h - 1; s += tmp[yy * w + x]; }
      a[y * w + x] = s / div;
    }
  }
}

// Aplica o misturador HSL suavizando o CAMPO DE FORÇA do ajuste (não a imagem).
// O granulado vem da força variar pixel-a-pixel pelo ruído de matiz/saturação
// do céu (pálido, cai na parte íngreme de satW). Borrando só os campos h/s/l a
// força fica lisa e o detalhe REAL da foto (bordas, textura, o gradiente do
// céu) fica 100% intacto.
function mixerAplicar(d, w, h, mix) {
  const n = w * h;
  const fH = new Float32Array(n), fS = new Float32Array(n), fL = new Float32Array(n);
  let algum = false;
  for (let i = 0, pi = 0; i < n; i++, pi += 4) {
    const a = mixerPesos(d[pi], d[pi + 1], d[pi + 2], mix);
    if (a && (a.h || a.s || a.l)) { fH[i] = a.h; fS[i] = a.s; fL[i] = a.l; algum = true; }
  }
  if (!algum) return;
  const rad = Math.max(2, Math.min(7, Math.round(Math.min(w, h) * 0.005)));
  blurCanal(fH, w, h, rad);
  blurCanal(fS, w, h, rad);
  blurCanal(fL, w, h, rad);
  for (let j = 0, pj = 0; j < n; j++, pj += 4) {
    if (!fH[j] && !fS[j] && !fL[j]) continue;
    const hsl = rgb2hsl(d[pj], d[pj + 1], d[pj + 2]);
    hsl[0] = (hsl[0] + (fH[j] / 100) * 0.1 + 1) % 1;
    hsl[1] = Math.max(0, Math.min(1, hsl[1] * (1 + fS[j] / 100)));
    hsl[2] = Math.max(0, Math.min(1, hsl[2] * (1 + fL[j] / 100)));
    const o = hsl2rgb(hsl[0], hsl[1], hsl[2]);
    d[pj] = c255(o[0]); d[pj + 1] = c255(o[1]); d[pj + 2] = c255(o[2]);
  }
}
...
export function aplicarPixels(d, w, h, p) {
  ...
  const luz     = { ...padrao.luz, ...(p?.luz || {}) };
  const cor     = { ...padrao.cor, ...(p?.cor || {}) };
  const efeitos = { ...padrao.efeitos, ...(p?.efeitos || {}) };
  const detalhe = { ...padrao.detalhe, ...(p?.detalhe || {}) };
  const curvaP  = p?.curva || padrao.curva;
  const mixerP  = p?.mixer || padrao.mixer;

  const lutRGB = curvaLUT(Array.isArray(curvaP?.rgb) ? curvaP.rgb : padrao.curva.rgb);
  const lutR   = curvaLUT(Array.isArray(curvaP?.r)   ? curvaP.r   : padrao.curva.r);
  const lutG   = curvaLUT(Array.isArray(curvaP?.g)   ? curvaP.g   : padrao.curva.g);
  const lutB   = curvaLUT(Array.isArray(curvaP?.b)   ? curvaP.b   : padrao.curva.b);

  const exposicao = (luz.exposicao || 0) / 100;
  const contraste = (luz.contraste || 0) / 100;
  const realces   = (luz.realces   || 0) / 100;
  const sombras   = (luz.sombras   || 0) / 100;
  const brancos   = (luz.brancos   || 0) / 100;
  const pretos    = (luz.pretos    || 0) / 100;

  const temp = (cor.temp || 0) / 100;
  const tint = (cor.tint || 0) / 100;
  const vib  = (cor.vibracao || 0) / 100;
  const sat  = (cor.saturacao || 0) / 100;

  const dehaze   = (efeitos.dehaze || 0) / 100;
  const hslAtivo = mixerP ? mixerTemAjuste(mixerP) : false;
  ...
  // 6) Misturador HSL — aplicado como campo de força suavizado (mata o granulado)
  if (hslAtivo) mixerAplicar(d, w, h, mixerP);
  ...
  if (efeitos.clareza)  claridade(d, w, h, efeitos.clareza / 100);
  if (efeitos.textura)  textura(d, w, h, efeitos.textura / 100);
  if (detalhe.nitidez)  nitidez(d, w, h, detalhe.nitidez / 100);
  if (detalhe.ruido)    reduzRuido(d, w, h, detalhe.ruido / 100);
  if (efeitos.vinheta)  vinheta(d, w, h, efeitos);
  if (efeitos.grao)     grao(d, w, h, efeitos);
}
...
export function aplicarEmCanvas(origem, p) {
  if (!origem || !origem.width || !origem.height) return origem;
  ...
  try {
    ...
    console.error('[aplicarEmCanvas] Erro ao aplicar ajustes:', err);
  }
...
export const ABAS_AJUSTE = [
  {
    id: 'luz', nome: 'Luz', icone: 'sun', grupo: 'luz',
    sliders: [
  ...
  {
    id: 'cor', nome: 'Cor', icone: 'palette', grupo: 'cor',
    // Os gradientes vêm do plugin: cada slider mostra, no próprio trilho, para
    // onde a imagem vai — quente/frio, verde/magenta, cinza/saturado.
    sliders: [
      { k: 'temp',      nome: 'Temperatura', min: -100, max: 100,
        grad: 'linear-gradient(90deg,#1f7fd0,#7fa0b8,#e8d840)' },
      { k: 'tint',      nome: 'Colorir',     min: -100, max: 100,
        grad: 'linear-gradient(90deg,#3aa84a,#a0a0a0,#c030b0)' },
      { k: 'vibracao',  nome: 'Vibração',    min: -100, max: 100,
        grad: 'linear-gradient(90deg,#9a9a9a 0%,#9a9a9a 38%,#4caf50 52%,#a8d040 68%,#e8c030 84%,#f08020 100%)' },
      { k: 'saturacao', nome: 'Saturação',   min: -100, max: 100,
        grad: 'linear-gradient(90deg,#9a9a9a 0%,#9a9a9a 38%,#4caf50 52%,#a8d040 68%,#e8c030 84%,#f08020 100%)' }
    ]
  },
  {
    id: 'efeitos', nome: 'Efeitos', icone: 'sparkles', grupo: 'efeitos',
    sliders: [
      { k: 'textura',        nome: 'Textura',    min: -100, max: 100 },
      { k: 'clareza',        nome: 'Claridade',  min: -100, max: 100 },
      { k: 'dehaze',         nome: 'Desembaçar', min: -100, max: 100 },
      // A partir daqui, a seção Vinheta — o título aparece antes deste slider.
      { k: 'vinheta',        nome: 'Quantidade', min: -100, max: 100, sec: 'Vinheta',
        grad: 'linear-gradient(90deg,#222,#888,#eee)' },
      ...
      // E aqui a seção Granulado.
      { k: 'grao',    nome: 'Quantidade', min: 0, max: 100, sec: 'Granulado' },
      { k: 'graoTam', nome: 'Tamanho',   min: 0, max: 100, filho: 'grao' },
      { k: 'graoAsp', nome: 'Aspereza',  min: 0, max: 100, filho: 'grao' }
    ]
  },
  {
    id: 'detalhe', nome: 'Detalhe', icone: 'adjustments', grupo: 'detalhe',
    sliders: [
...
// ── Equilíbrio de branco por conta-gotas ──
//
// A pessoa clica num ponto que DEVERIA ser cinza/branco neutro. Se ali há uma
// dominante de cor (mais azul que vermelho, por exemplo), a imagem está fria —
// e devolvemos a temperatura/tint que a neutralizam.
//
// r, g, b são a cor ORIGINAL do pixel (sem ajustes já aplicados). Retorna
// { temp, tint } na faixa -100..100.
export function equilibrioDeBranco(r, g, b) {
  let media = (r + g + b) / 3;
  if (media < 2) media = 2;

  const difTemp = (b - r) / media;      // azul>vermelho → fria → esquentar (temp+)
  const difTint = (g - media) / media;  // verde em excesso → puxar para magenta

  return {
    temp: Math.max(-100, Math.min(100, Math.round(difTemp * 120))),
    tint: Math.max(-100, Math.min(100, Math.round(-difTint * 120)))
  };
}
