...
export const RATIOS_CROP = [
  'livre', 'original', '1:1', '16:9', '9:16', '4:3', '3:4',
  '3:2', '2:3', '21:9', '4:5', '5:4'
];



// ── Apagar os pixels cortados ──
//
// O corte, por si só, não destrói nada: ele encolhe o DOCUMENTO, e as camadas
// continuam inteiras, transbordando para fora da nova borda. É o que o
// Photoshop faz com "Apagar pixels cortados" desmarcado — e é por isso que lá
// dá para arrastar a camada e revelar o que tinha ficado de fora.
//
// Esta função é a caixinha MARCADA: ela reescreve o canvas de cada camada com
// só a parte que sobrou dentro do documento. O que estava fora some de verdade.
export function apagarForaDoDoc(l, docW, docH) {
  if (l.tipo === 'grupo') return l;

  const ew = largura(l);
  const eh = altura(l);

  // A parte da camada que caiu dentro do documento
  const ix0 = Math.max(l.x, 0);
  const iy0 = Math.max(l.y, 0);
  const ix1 = Math.min(l.x + ew, docW);
  const iy1 = Math.min(l.y + eh, docH);

  const iw = ix1 - ix0;
  const ih = iy1 - iy0;

  // A camada ficou inteira de fora: some.
  if (iw <= 0 || ih <= 0) return null;

  // Já estava toda dentro: não há o que apagar, e reescrever o canvas à toa só
  // custaria memória e uma reamostragem desnecessária.
  const dentro = l.x >= -0.5 && l.y >= -0.5
              && l.x + ew <= docW + 0.5 && l.y + eh <= docH + 0.5;
  if (dentro) return l;

  // O novo canvas guarda a RESOLUÇÃO ORIGINAL da parte que sobrou. Uma camada
  // 4K exibida a 30% não deve virar 30% de pixels ao ser cortada — ela seria
  // destruída sem que ninguém pedisse.
  ...
  cx.drawImage(l.canvas, sx, sy, iw * escX, ih * escY, 0, 0, rw, rh);

  // A máscara acompanha o mesmo corte, ou deixaria de bater com o pixel.
  let masc = null;
  if (l.mascara) {
    const mx = l.mascara.width  / l.canvas.width;
    const my = l.mascara.height / l.canvas.height;

    masc = canvasVazio(Math.max(1, Math.round(rw * mx)),
                       Math.max(1, Math.round(rh * my)));

    masc.getContext('2d').drawImage(
      l.mascara,
      sx * mx, sy * my, iw * escX * mx, ih * escY * my,
  ...
  return {
    ...
    canvas: c,
    mascara: masc,
    // O pixel virgem foi cortado junto: já não corresponde ao canvas, e os
    // Ajustes que partissem dele reapareceriam com as bordas de volta.
    original: null,
    x: ix0,
    y: iy0,
    // A escala na tela não muda: o pedaço ocupa o mesmo espaço que ocupava.
    escala:  iw / rw,
    escalaY: ih / rh
  };
}
