...
export async function salvarLeitura({ origem, titulo, materiais, geracaoId, thumb, chaveImagem }) {
  ...
  try {
    const r = await fetch(`${RENDER_URL}/leituras`, {
      method: 'POST',
      headers: await comToken(),
      // chaveImagem agrupa a edição como nova VERSÃO da mesma imagem na aba Análises
      body: JSON.stringify({ origem, titulo, materiais, geracaoId, thumb, chaveImagem })
    });
    if (!r.ok) return null;
    const d = await r.json();
    return d.id;
