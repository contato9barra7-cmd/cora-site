...
export function selecaoRapida(sel, modo, pixels, w, h, p, tolerancia, raio) {
  ...
  const grad = (ii) => {
    // Clampa os vizinhos aos limites da imagem: sem isso, na linha y=0 o índice
    // ii - w*4 ficava negativo (pixels[neg] = undefined → lum = NaN → grad = NaN),
    // e nas colunas x=0/x=w-1 o ii±4 vazava para a linha vizinha (wraparound). O
    // NaN reprovava `g < bordaLimiar` e o flood parava uma linha antes da borda.
    const p = ii >> 2;               // ii / 4
    const x = p % w, y = (p / w) | 0;
    const L = lum(ii);
    const lx1 = lum((y * w + Math.min(w - 1, x + 1)) * 4);
    const lx0 = lum((y * w + Math.max(0, x - 1)) * 4);
    const ly1 = lum((Math.min(h - 1, y + 1) * w + x) * 4);
    const ly0 = lum((Math.max(0, y - 1) * w + x) * 4);
    const gx = Math.abs(lx1 - L) + Math.abs(L - lx0);
    const gy = Math.abs(ly1 - L) + Math.abs(L - ly0);
    return gx + gy;
  };
...
export function pincelada(alvo, de, para, opts) {
  ...
  for (const pt of pts) {
    // ── A dureza 100 não é um gradiente ──
    //
    // A dureza é o raio do miolo sólido; o resto é o esfumado. Mas a 100% o
    // miolo ocupa o círculo inteiro, e `createRadialGradient(r, ..., r)` tem os
    // dois raios IGUAIS — um gradiente degenerado, que pela especificação do
    // Canvas não pinta absolutamente nada.
    //
    // Era por isso que o pincel sumia justamente onde deveria ser mais nítido.
    if (dureza >= 100) {
      cx.fillStyle = `rgba(${c},1)`;
    } else {
      const g = cx.createRadialGradient(pt.x, pt.y, raio * (dureza / 100),
                                        pt.x, pt.y, raio);
...
export function desfoqueMovimento(canvas, dist, angulo, sel) {
  ...
  // ── O TETO de amostras ──
  //
  // Este blur soma, para cada pixel, as amostras ao longo do rastro. Isso é
  // O(pixels × distância) — e com distância 200 numa imagem de um milhão de
  // pixels dá centenas de milhões de operações, que travam a aba.
  //
  // Mas a média de 400 amostras é quase igual à média de 60 amostras BEM
  // ESPAÇADAS: o olho não distingue. Então, quando o rastro é longo, pulamos de
  // `passo` em `passo` — o resultado fica visualmente o mesmo e o custo deixa de
  // crescer com a distância.
  const MAX_AMOSTRAS = 64;
  const passo = Math.max(1, Math.ceil((2 * n + 1) / MAX_AMOSTRAS));

  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      let r = 0, g = 0, b = 0, a = 0, cont = 0;

      for (let k = -n; k <= n; k += passo) {
