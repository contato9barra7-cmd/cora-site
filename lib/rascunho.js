...
const MAX_REFS_BYTES  = 3_000_000;  // teto p/ as imagens de referência juntas
...
export function salvarRascunho(aba, estado) {
  ...
  try {
    ...
    // Referências: guarda só o base64 (a prévia é reconstruída no reload), com
    // um teto de tamanho — o localStorage não aguenta várias imagens 2K juntas.
    // As que não couberem ficam de fora (a pessoa re-sobe essas, não todas).
    if (Array.isArray(dados.refs)) {
      let accRefs = 0;
      dados.refs = dados.refs
        .map((r) => {
          const b = r && r.base64 ? r.base64 : null;
          if (!b || accRefs + b.length > MAX_REFS_BYTES) return null;
          accRefs += b.length;
          return { base64: b };
        })
        .filter(Boolean);
    ...
    // Estourou a cota do localStorage. Prioridade: manter a IMAGEM BASE.
    // 1º tenta sem as referências (mantendo a imagem, trimada se for enorme).
    const semRefs = { ...estado, refs: [], salvoEm: Date.now() };
    if (semRefs.imagem && semRefs.imagem.length > MAX_IMG) {
      semRefs.imagem = null; semRefs.previa = null; semRefs.imagemGrande = true;
    }
    try {
      localStorage.setItem(CHAVE, JSON.stringify(semRefs));
    } catch {
      // 2º nem a imagem coube: salva só o resto (materiais, luz, entorno…).
      try {
