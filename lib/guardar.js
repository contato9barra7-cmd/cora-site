...
// ── PRIVACIDADE: a chave carrega a CONTA logada ──
// Sem isso, num mesmo navegador (ou ao trocar de conta), a conta B abria o
// trabalho de pós da conta A. A chave 'atual_<idDaConta>' isola cada conta.
function contaId() {
  try {
...
function chaveAtual() {
  return 'atual_' + contaId();
}

// Remove o rascunho ANTIGO (chave 'atual', sem conta) que ficou de antes desta
// correção — ele podia ser de outra conta. Roda uma vez por sessão.
let _legadoLimpo = false;
async function limparLegado(db) {
  if (_legadoLimpo) return;
  _legadoLimpo = true;
  try {
    await new Promise((ok) => {
      const t = db.transaction(LOJA, 'readwrite');
      t.objectStore(LOJA).delete('atual');
      t.oncomplete = ok;
      t.onerror    = ok;
    });
  } catch (e) { /* não vale travar por causa da limpeza */ }
}
...
export async function salvar(camadas, med, selecao, aindaVale) {
  ...
  for (const l of camadas) {
    const c = {
      ...
      // A receita dos filtros inteligentes. Sem ela, o rascunho recuperado viria
      // com os efeitos carimbados e sem volta.
      filtros: []
    };
    ...
    if (l.tipo !== 'grupo') {
      ...
      // Cada filtro, com a sua máscara em Blob.
      for (const f of (l.filtros || [])) {
        c.filtros.push({
          ...
          mascara: f.mascara ? await paraBlob(f.mascara) : null
        });
      }
    }
  ...
  await limparLegado(db);

  await new Promise((ok, erro) => {
    const t = db.transaction(LOJA, 'readwrite');
    t.objectStore(LOJA).put(pacote, chaveAtual());
    t.oncomplete = ok;
...
export async function carregar() {
  ...
  const pacote = await new Promise((ok, erro) => {
    ...
    const r = t.objectStore(LOJA).get(chaveAtual());
  ...
  // Os ids são refeitos, como na importação de um .crd: os do rascunho vêm de
  // outra sessão, e reaproveitá-los arrisca colidir com um id criado agora.
  const novosIds = {};
  for (const c of pacote.camadas) novosIds[c.id] = novoId();
  ...
  for (const c of pacote.camadas) {
    ...
    if (c.tipo !== 'grupo') {
      ...
      for (const f of (c.filtros || [])) {
        l.filtros.push({
          ...f,
          mascara: f.mascara ? await paraCanvas(f.mascara) : null
        });
      }
    } else {
      l.canvas = null;
...
export async function apagar() {
  ...
  await new Promise((ok) => {
    const t = db.transaction(LOJA, 'readwrite');
    t.objectStore(LOJA).delete(chaveAtual());
    t.oncomplete = ok;
...
export async function existe() {
  try {
    ...
    const n = await new Promise((ok, erro) => {
      const t = db.transaction(LOJA, 'readonly');
      const r = t.objectStore(LOJA).count(chaveAtual());
      r.onsuccess = () => ok(r.result);
      r.onerror   = () => erro(r.error);
    });
