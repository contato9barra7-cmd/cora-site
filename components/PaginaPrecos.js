...
// ═══════════════════════════════════════════════════════════════════════════
//  PÁGINA DE PREÇOS (COMPONENTE COMPARTILHADO)
//
//  Usado por app/precos/page.js, app/es/precos/page.js e app/en/precos/page.js.
//  Recebe a prop `idioma` para formatar números e orientar a navegação.
// ═══════════════════════════════════════════════════════════════════════════
...
import CartaoPlano, { brl, brlInt } from './CartaoPlano';
...
function num(v, idioma = 'pt') { return typeof v === 'number' ? v.toLocaleString(localeDeIdioma(idioma)) : v; }
...
export default function PaginaPrecos({ idioma: propIdioma = 'pt' }) {
  ...
  return (
    ...
    <main className="pr">
      <Cabecalho aqui="precos" idioma={idioma} />
            ...
            <table className="custo">
              ...
              <tbody>
                {custos[abaCusto].linhas.map((linha, i) => (
                  <tr key={i}>
                    {linha.map((v, j) => (
                      <td key={j} className={j === 0 ? '' : 'num'}>{j === 0 ? tr(v) : num(v, idioma)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
          ...
          <div className="recargas">
            {recargas.map((r) => (
              <div key={r.id} className={'recarga' + (r.popular ? ' recarga--pop' : '')}>
                ...
                <div className="recarga__cred">{r.creditos.toLocaleString(localeDeIdioma(idioma))} {t('precos_creditos')}</div>
