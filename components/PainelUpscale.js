...
export default function PainelUpscale({
  ...
  async function gerar() {
    if (!base) { setErro(t('painelupscale_erro_suba_imagem')); return; }
    setErro('');
    ...
    try {
      ...
      if (!r.imagem) setErro(t('painelupscale_erro_falhou'));
    } catch (e) {
      setErro(e.message);
    } finally {
    ...
    <div className="up-painel">
      ...
      <section className="up-bloco">
        ...
        <div className="up-preset-row">
          ...
          />
          <button className="up-mini" onClick={abrirModalPreset} title={t('painelupscale_salvar_preset_atual')} aria-label={t('painelupscale_salvar_preset')}>+</button>
          <button className="up-mini up-mini--danger" onClick={excluirPreset} title={t('painelupscale_excluir_preset')} aria-label={t('painelupscale_excluir_preset')} disabled={!presetAtual}>
      ...
      <PickerImagem
        ...
        titulo={t('painelupscale_titulo_picker')}
      ...
      {modalPreset && (
        <div className="up-modal-fundo" onClick={() => setModalPreset(false)}>
          <div className="up-modal" onClick={(e) => e.stopPropagation()}>
            <p className="up-modal-titulo">{t('painelupscale_nome_preset')}</p>
            <input
              type="text"
              ...
              placeholder={t('painelupscale_ph_nome_preset')}
              autoFocus
            />
            <div className="up-modal-botoes">
              <button className="up-modal-b" onClick={() => setModalPreset(false)}>{t('comum_cancelar')}</button>
              <button className="up-modal-b up-modal-b--on" onClick={confirmarPreset} disabled={!nomePreset.trim()}>{t('comum_salvar')}</button>
            </div>
          </div>
        </div>
      )}
